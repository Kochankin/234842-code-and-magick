...
function drawPolygon(context, coordinates, fillStyle, strokeStyle) {
  context.beginPath();
  context.moveTo(coordinates[0].xInit, coordinates[0].yInit);
...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var j = 0; j < times.length; j++) {
    rectX += 90;
    var rectH = columnHeights[j];
    var rectY = CANVASHEIGHT - rectH - SHIFT;
    var random = getRandomInt(0, columnColors.length);
    ctx.fillStyle = (names[j] === 'Вы') ? 'rgba(255, 0, 0, 1)' : columnColors[random];
    ctx.fillRect(rectX, rectY, RECTW, rectH);
    ctx.font = '13px PT Mono';
    ctx.fillStyle = 'rgb(0, 0, 0)';
    textX += 90;
    ctx.fillText(names[j], textX, 250);
    var textY = 230 - rectH;
